...
### 主界面 - 模型响应
多个模型并行响应用户问题，实时展示各模型的回答。

![模型响应截图](docs/1_模型响应截图.png)

### 匿名评审阶段
各模型对其他模型的回答进行匿名评分，确保评价的客观性。

![匿名评分截图](docs/2_匿名评分截图.png)

### 主席综合答案
主席模型综合所有回答和评分，生成最终的高质量答案。

![综合答案截图](docs/3_综合答案截图.png)

### 评分排名
根据匿名评审结果，展示各模型的得分排名。

![评分截图](docs/4_评分截图.png)

### 模型选择
灵活选择参与讨论的模型，支持多模型协作。

![模型选择截图](docs/模型选择截图.png)

### 模型配置
在前端界面直接配置模型参数和 API 密钥。

![模型配置截图](docs/模型配置截图.png)

### 文件管理
上传和管理文档文件，支持多种格式（需配置 MinerU）。

![文件管理截图](docs/文件管理截图.png)

### 系统设置
配置系统参数，包括温度、并发数、MinerU 等。

![系统设置截图](docs/系统设置截图.png)
